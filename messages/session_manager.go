...
import (
	...
	qrcodeTerminal "github.com/normen/whatscli/qrcode"
...
func (sm *SessionManager) loginWithQRCode(client *whatsmeow.Client) error {
	...
	for evt := range qrChan {
		switch evt.Event {
		case "code":
			// --- ASCII QR en terminal ---
			terminal := qrcodeTerminal.New()
			terminal.SetOutput(tview.ANSIWriter(sm.uiHandler.GetWriter()))
			terminal.Get(evt.Code).Print() // imprime en ASCII

			// --- PNG QR en archivo ---
			// tamaño fijo en píxeles (ej. 256)
			err := qrcode.WriteFile(evt.Code, qrcode.Medium, 256, "qr.png")
			if err != nil {
				sm.uiHandler.PrintText(fmt.Sprintf("Error generating PNG QR: %v", err))
			} else {
				sm.uiHandler.PrintText("QR code also saved as qr.png")
			}
